...
The third reference is the white canvas template – you MUST match its exact dimensions and aspect ratio precisely. 

Important composition rules:
- Only ONE person (subject) should appear in the final image — do NOT duplicate or mirror the same person.
- Avoid showing the same person and product twice in different poses or sides.
- Fill the entire canvas 1024*1024 fully (edge-to-edge, full-bleed composition).
- The product and person should be natural, cohesive, and clearly branded.
- Do not leave any white areas, borders, padding, or margins.
- Avoid framed, mockup, or poster-style layouts.

Do not include text duplication, extra logos, or repeated visual elements. Only one person, one product, and one logo should be shown in the final image.
        .
        `
        : prompt;
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Step 6: Download logo, OG image, and white canvas in parallel
    console.log(
      '📥 Downloading logo, OG image, and white canvas in parallel...'
    );
    ...
    const [logoBase64, ogImageBase64, whiteCanvasBase64] =
      await Promise.all(downloadPromises);

    // Step 7: Generate image with Gemini 2.5 Flash (using available references)
    console.log('🌐 Generating image with Gemini 2.5 Flash...');
    ...
    if (!generatedImageUrl) {
      return NextResponse.json(
        { error: 'Failed to generate image with Flux pro v1.1' },
        { status: 500 }
      );
    }

    // Step 8: Prepare reference images for Gemini (already used above) – no post-editing
