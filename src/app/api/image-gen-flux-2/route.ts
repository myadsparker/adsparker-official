...
export async function POST(req: NextRequest) {
  try {
    ...
    // For image editing, we'll pass the logo as input_image reference
    // The prompt describes what to create, and FLUX.2 will use the logo from input_image
    let effectivePrompt = promptData.prompt;
    
    // Add instructions to include the logo from the input image
    if (logoBase64) {
      effectivePrompt = `${effectivePrompt} Include the brand logo from the input image once, small and tasteful (e.g., a corner watermark), never duplicated.`;
    }
    ...
    // Add logo as input_image for image editing (if available)
    // FLUX.2 image editing uses input_image as the base/reference image
    // Reference: https://docs.bfl.ai/flux_2/flux2_image_editing
    // input_image accepts URL or base64 string
    if (logoBase64) {
      // Pass base64 string directly (BFL API accepts base64 for input_image)
